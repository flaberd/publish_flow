...
**Мінімальний розмір root EBS-тому: 20GB.** Дефолтні 8GB (стандартний
розмір для більшості Ubuntu AMI, якщо явно не вказати інше при запуску
інстансу) перевірено не вистачає — сама ОС + Docker Engine (без жодних
даних застосунку) займають ~5GB baseline, і вже за кілька деплоїв
(нові image-теги + MySQL-дані + відео) диск заповнюється настільки, що
`docker compose pull` падає з `no space left on device` посеред
деплою. Різниця в ціні між 8GB і 20GB — копійки (EBS gp3 тарифікується
за GB на місяць, ~$0.08/GB — тобто різниця ~$1/місяць), тому сенсу
економити немає. Якщо цей крок пропущено при первинному запуску,
збільшити том можна пізніше без даунтайму й перестворення інстансу:
змінити розмір EBS-тому в AWS Console (EC2 → Volumes → Modify Volume),
після чого на самому хості `sudo growpart /dev/nvme0n1 1 &&
sudo resize2fs /dev/nvme0n1p1` (або `xfs_growfs /`, якщо файлова
система xfs) — і `df -h /` одразу покаже новий розмір.
...
3. Job **deploy**: SSH на EC2 (`appleboy/ssh-action`), на хості:
   - `docker image prune -af` — прибирає образи, які не використовує
     жоден поточний контейнер (тобто все, крім того, що зараз реально
     запущено), перед тим як тягнути новий. Без цього кроку старі
     `sha-<commit>`-теги накопичуються на диску EC2 нескінченно (у GHCR
     вони й так залишаються назавжди — це не про втрату можливості
     відкату, а про диск хоста);
...
залишається доступною після наступного деплою назавжди, незалежно від
того, що зараз лежить (чи не лежить) на диску самого EC2.

Локально на EC2 при цьому навмисно тримається лише те, що зараз реально
запущено (розділ 8: `docker image prune -af` перед кожним pull) — для
цього проєкту швидкість відкату не критична, тож сенсу вічно займати диск
хоста образами "про всяк випадок" немає. Ціна цього рішення: відкат на
старий тег тягне його заново з GHCR (секунди-десятки секунд залежно від
розміру образу), а не піднімає з локального кешу миттєво — але це й досі
на порядок швидше за повний ребілд.
...
одразу перевикочує вказаний тег на EC2 — без ребілду (образ підтягується
з GHCR).
